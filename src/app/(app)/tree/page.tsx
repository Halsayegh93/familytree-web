...
export default async function TreePage() {
  ...
  const { data: members } = await applyCountableFilters(
    supabase
      .from("profiles")
      .select("id, first_name, full_name, father_id, mother_id, role, status, avatar_url, is_deceased, is_married, is_hidden_from_tree, sort_order, birth_date, death_date, phone_number")
  ...
  if (canModerate) {
    const [{ data: w }, { data: ext }, { data: wr }] = await Promise.all([
      supabase
        .from("women_members")
        .select("id, first_name, full_name, parent_id, mother_id, mother_name, husband_id, gender, is_deceased, is_married, birth_date, death_date, avatar_url, sort_order")
        .limit(10000),
      supabase
